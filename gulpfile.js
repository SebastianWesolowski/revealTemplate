...
gulp.task('plugins', () => {
    return Promise.all([
        ...
        {
            name: 'RevealExternal',
            input: './node_modules/reveal_external/external/external.js',
            output: './plugin/external/external'
        },
    ].map(plugin => {
        return rollup({
...
gulp.task("css-customs", () =>
    gulp
        .src(["./css/custom/*.{sass,scss}"])
        .pipe(compileSass())
...
gulp.task('css', gulp.parallel('css-themes', 'css-core', 'css-customs'))
